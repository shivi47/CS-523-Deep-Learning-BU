...
# make the ground truth classes
classes: np.ndarray = np.random.randint(out_dim, size=num_examples)

# turn classes into one-hot vectors
Y_gt: np.ndarray = np.zeros((num_examples, out_dim), dtype=float)
Y_gt[np.arange(num_examples), classes] = 1


# one hidden layer and softmax predictions
